...
- [x] 2.1 注册 tab：`ctx.slots.inject('settings.plugins.tab', () => ctx.slots.register({ name:'settings.plugins.tab', id:'subagents', order:50, label: () => t('子代理'/'Subagents'), locale: NS }, OrphansTab))`；槽缺失或注册抛错时静默降级并写 `__dsew.orphanTabRegistered`。验证：harness 断言槽名/id/order/标签双语，且「无该槽」现场不抛未捕获错误。
...
- [x] 3.1 `dsh-plugin-manager/src/client.js`：注册行改为 `settings.plugins.tab`（id `local-plugins`，order 20），组件与服务注入不变。验证：`node dsh-plugin-manager/test/bundle.test.mjs` 断言槽名/id/order，且**不再**注册 `settings.section`。
...
- [x] 5.1 `sub-plugins/dsh-esc-rewind/README.md` 增「子代理回收」段（入口 = 设置 → 插件 → 子代理；停止/找回语义与边界；全手动）；`ACCEPTANCE.md` 增人工项 6.x（tab 出现、统计与探针一致、停止在跑孤儿、找回有完整回合的孤儿、空态、全手动无副作用）。
- [x] 5.2 三个插件 README/ACCEPTANCE 记账「设置入口已移到 设置 → 插件 → <tab>」。
- [x] 5.3 `docs/knowledge/2026-09-08-dsh-esc-rewind.md` 新增 v7.7 小节（可复用结论：`settings.plugins.tab` 是核心声明的列表槽、tab 懒挂载、注册范式与 `settings.section` 的取舍；孤儿定义=可达性；找回=fork 成普通会话），并在 `docs/knowledge/README.md` 追加索引行。
...
- [ ] 6.1 重启 `dsh web` + 刷新页面：设置 → 插件 出现「子代理」tab，统计与本机现场（27 个孤儿，含 `6455048f`）一致；`GET /__esc-rewind/orphans` 探针结论一致。
- [ ] 6.2 对仍在跑的孤儿执行「停止」（确认静默），对 1 个有完整回合的孤儿执行「找回」（确认新会话出现在侧栏、内容可继续、原孤儿仍在）。
- [ ] 6.3 设置左侧导航只剩核心「插件」一行；「本地插件」「Hindsight 模型」「空闲通知」三处内容出现在对应 tab 内且功能不变。