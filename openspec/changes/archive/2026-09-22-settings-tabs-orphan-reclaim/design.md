...
- 本仓插件的设置内容收拢为「本地插件」一级入口里的 tab，设置导航只留核心一行。
...
**D1 入口使用核心 `settings.localPlugins.tab`，不自建入口。**
...
**D9 迁移只改注册行**：槽名换成 `settings.localPlugins.tab`、补 `id`/`order`/`label`，组件主体与其服务注入不变；若某面板依赖 `settings.section` 的 owner props（如 `t`），改用注册项 `locale: NS` 提供的 `props.t` 或 `ctx.locale.bind(NS)` —— 实施时按各插件现状做最小改动。

## Risks / Trade-offs

- [设置导航少三行会被用户当成功能消失] → 三个插件的 README/ACCEPTANCE 明确写「入口已移到 设置 → 本地插件」，提交信息点明用户可见位置变化。
...
- [核心版本差异（无 `settings.localPlugins.tab`）] → 注册被捕获并跳过，诊断字段（`__dsew.orphanTabRegistered`、`/status`）可见原因。
