...
6. DOM 锚点：核心“视图选项”按钮 aria-label = `视图选项`/`View options`（随 locale）；CSS Modules 子串 class `[class*=headerActions]`/`[class*=sectionHeader]`/`[class*=listArea]`/`[class*=flatList]`/`[class*=searchExpanded]`/`[class*=searchTree]`；无稳定 id/data-*。
