...
| 子插件可解析 | profile `devDependencies`（`link:` 到仓库子目录） | pnpm 安装后从 profile `node_modules` 解析；`dsh plugin` 的 reconcile 只扫 `dependencies`，所以**不会**把它们塞回 bundles |
...
| 面板 | 设置页一级入口「本地插件」（`settings.section`，order 16） | 行 = 状态点 + 包名/目录 + 描述 + 主开关 + 移除；顶部 = 仓库路径 / 目标 profile / 刷新 / 一键迁移 |
...
- 仓库目录：`path.resolve(本插件目录, '..', '..')`（即 `dsh-plugin-manager` 的上一级仓库根），目录被移动/删除时面板给出可读错误。
