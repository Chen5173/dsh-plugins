...
前置：`src/index.js` 改动后已重启 GUI host；已刷新页面（`window.__dsew.orphanTabRegistered === true`）。

- [ ] 6.1 `[B]` 设置左侧导航出现**「本地插件」一级入口**（核心「插件」行不受影响）；进入后 tab 列表出现「子代理」，点开显示计数与列表（label、父 id、是否在跑、最后活动时间）
- [ ] 6.1b `[B]` **按钮与布局**：工具栏与每行按钮为细边框圆角胶囊（与核心「插件」卡片同款），「停止」「删除」为红色描边；每行的「停止/找回/删除」**右对齐**，左侧两行为「名称 + 运行中/父会话」；选中态下批量按钮可用
- [x] 6.8 自动化：孤儿删除端点——空 ids / 非法 id → 400；守卫拒绝（自己还挂着 1 个子代理）→ `{deleted:false, reason:'subagents', children:1}` 且**磁盘零改动**；未受守卫的孤儿被**真删**（临时 `DSH_HOME` 下日志目录消失）；响应含 `deleted`/`blocked` 计数（`bundle.test`「孤儿删除端点」）。
- [x] 6.9 自动化：面板「删除」——逐行删除可点、未勾选时「删除选中」禁用；**全选 → 取消全选** 正确切换并影响批量可用性；点「删除选中」发出 `{ids:[…]}`、确认框写明数量与「不可恢复」；结果如实渲染「已删除」与「未删除：自己还挂着 2 个子代理（请先删子级）」（`bundle.test`「删除…全选→批量」）。
- [x] 6.10 自动化：**取消确认不发任何请求**、不产生删除结果文案（`bundle.test`「取消删除确认」）。
- [x] 6.12 自动化：宿主半缺端点（404/405，且无 JSON body）时，面板显示「宿主半没有这个端点…需要重启 dsh web」而不是裸错误码 `delete-failed-405`（`bundle.test`「宿主半缺端点…」）。
- [ ] 6.13 `[B]` 排障一致性：改动宿主半后**未重启** `dsh web` 时点删除 → 应看到重启提示；重启后同一动作成功。`GET /__esc-rewind/status` 的 `endpoints` 用来核对当前进程实际注册的路由。
- [ ] 6.11 `[B]` 真机：对一个普通孤儿点「删除」→ 确认 → 该行消失、`~/.dsh/sessions/**/<id>` 目录消失；对一个仍有子代理的孤儿点「删除」→ 拒绝并显示子级数量，先删子级后可删掉它。
- [ ] 6.2 `[B]` 面板统计与真相同步：`GET /__esc-rewind/orphans` 的 `count` 与面板一致（2026-09-22 本机现场为 27，含仍在跑的 `6455048f`）
- [ ] 6.3 `[B]` **全手动**：打开面板与点「刷新统计」只发 GET（Network 无 POST），孤儿的 `running` 状态不变
- [ ] 6.4 `[B]` 对在跑的孤儿点「停止」→ 行内结果给出「已停止」或「已请求取消，未确认静默（timeout）」；刷新统计后该行不再显示「运行中」
- [ ] 6.5 `[B]` 对有完整回合的孤儿点「找回」→ 出现一个新的**普通会话**（侧栏可见、标题=孤儿标签、内容是孤儿到该回合为止的完整对话）并被自动打开；原孤儿仍在库中；`__dsew.orphanActions` 记录一次 rescue
- [ ] 6.6 `[B]` 没有完整回合的孤儿：「找回」按钮禁用且 title 为「没有完整回合，无法找回」；批量按钮未勾选时禁用，勾选后可用
- [ ] 6.7 `[B]` 空态/失败态如实呈现：孤儿 0 个显示「没有孤儿子代理」；宿主端点失败显示「读取失败：<原因>」（不得假装 0 个，`__dsew.orphans.count === null`）
