...
> **2026-09-10 真机走查记录**：用户按 spec 的 6 需求 / 18 场景走查一遍，结论"除已修复的问题外没其他问题"。本轮发现并修复：对**已在文件管理器中打开**的目录重复点击没有任何可见变化（宿主确实成功，是 Windows 复用既有窗口）→ 保持复用行为、成功文案改为「该目录已在文件管理器中打开」，详见变更 `add-open-session-workdir`（2026-09-10 归档）。本清单的 `[H]`/`[B]` 条目**仍未逐条执行**——走查按 spec 场景进行，未按本清单逐条留痕。
